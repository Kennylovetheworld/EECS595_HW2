...
class data_process():
    ...
    def data_embedding(self, neg_s, pos_s, validation = False, test = False):
        ...
        if validation is True:
            outname = 'log/data_dan_doc_validation'+'.pkl'
        elif test is True:
            outname = 'log/data_dan_doc_test'+'.pkl'
        else:
            outname = 'log/data_dan_doc'+'.pkl'
        if os.path.isfile(outname):
            ...
            for sent in neg_s:
                ...
                for i, word in enumerate(sent):
                    ...
                    length += 1
                    mean_vec += vec
                embedded_sents.append(mean_vec/length)
            for sent in pos_s:
                length = 0
                label.append(1)
                sent = word_tokenize(sent)
...
class DAN(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(300, 600)
        self.linear2 = nn.Linear(600, 600)
        self.linear3 = nn.Linear(600, 2)
...
for epoch in range(300):
    ...
    for iteration in range(20):
        net.zero_grad()
        #Choose a Batch
        indexs = [index % features.shape[0] for index in range(iteration*BATCH_SIZE, (iteration + 1)*BATCH_SIZE)]
        x = features[indexs] 
        y = labels[indexs]
    ...
    if accuracy > max_acc:
        max_acc = accuracy
        torch.save(net, "DAN_best.torch")
    
#Evaluate the accuracy on the test set
net = torch.load("DAN_best.torch")
net.eval()
